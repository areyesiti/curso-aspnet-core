...
namespace CoreGram.Repositories
{
    public class LikeRepository
    {
        ...
        public LikeDto Like(LikeDto dto)
        {
            // Obtención de un like para un usuario y un post con Linq
            //var model = _context.Likes.FirstOrDefault(x => x.UserId == dto.UserId && x.PostId == dto.PostId);

            // Obtención de un like para un usuario y un post FromSql
            var model = _context.Likes
                .FromSql("SELECT * FROM Likes WHERE UserId = {0} AND PostId = {1}", dto.UserId, dto.PostId)
                .FirstOrDefault();
